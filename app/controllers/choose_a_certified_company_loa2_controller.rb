...
class ChooseACertifiedCompanyLoa2Controller < ApplicationController
  ...
  def index
    ...
    @recommended_idps = @recommended_idps.reject(&:unavailable) + @recommended_idps.select(&:unavailable)
    @non_recommended_idps = IDENTITY_PROVIDER_DISPLAY_DECORATOR.decorate_collection(suggestions[:unlikely])
    @non_recommended_idps = @non_recommended_idps.reject(&:unavailable) + @non_recommended_idps.select(&:unavailable)
